...
<head>
    ...
    <title>{{ config('app.name', 'عرب للإدارة') }}</title>
    <script>
        (function() {
...
<body class="font-sans antialiased bg-surface text-on-surface">
    <div class="flex min-h-screen">
        <nav class="hidden lg:flex flex-col h-screen fixed right-0 top-0 border-l border-outline-variant bg-surface-container-low w-64 z-50">
            <div class="p-lg">
                <h1 class="font-headline-md text-headline-md text-primary tracking-tight">عرب للإدارة</h1>
                <p class="font-label-sm text-on-surface-variant mt-xs">مدير الأسطول</p>
            </div>
            <div class="mt-md flex-grow space-y-xs px-sm">
                <a href="{{ route('admin.dashboard') }}" class="flex items-center gap-sm px-md py-sm rounded-lg {{ request()->routeIs('admin.dashboard') ? 'active-nav-bg text-secondary font-bold' : 'text-on-surface-variant font-medium' }} transition-all hover:bg-surface-container-high duration-200">
                    <span class="material-symbols-outlined">dashboard</span>
                    <span class="font-label-md text-label-md">لوحة التحكم</span>
                </a>
                <a href="{{ route('admin.cars') }}" class="flex items-center gap-sm px-md py-sm rounded-lg {{ request()->routeIs('admin.cars') ? 'active-nav-bg text-secondary font-bold' : 'text-on-surface-variant font-medium' }} transition-all hover:bg-surface-container-high duration-200">
                    <span class="material-symbols-outlined">directions_car</span>
                    <span class="font-label-md text-label-md">إدارة الأسطول</span>
                </a>
                <a href="{{ route('admin.bookings') }}" class="flex items-center gap-sm px-md py-sm rounded-lg {{ request()->routeIs('admin.bookings') ? 'active-nav-bg text-secondary font-bold' : 'text-on-surface-variant font-medium' }} transition-all hover:bg-surface-container-high duration-200">
                    <span class="material-symbols-outlined">calendar_month</span>
                    <span class="font-label-md text-label-md">إدارة الحجوزات</span>
                </a>
                <a href="{{ route('admin.control-center') }}" class="flex items-center gap-sm px-md py-sm rounded-lg {{ request()->routeIs('admin.control-center') ? 'active-nav-bg text-secondary font-bold' : 'text-on-surface-variant font-medium' }} transition-all hover:bg-surface-container-high duration-200">
                    <span class="material-symbols-outlined">edit_note</span>
                    <span class="font-label-md text-label-md">مركز التحكم</span>
                </a>
                <a href="{{ route('admin.users') }}" class="flex items-center gap-sm px-md py-sm rounded-lg {{ request()->routeIs('admin.users') ? 'active-nav-bg text-secondary font-bold' : 'text-on-surface-variant font-medium' }} transition-all hover:bg-surface-container-high duration-200">
                    <span class="material-symbols-outlined">group</span>
                    <span class="font-label-md text-label-md">المستخدمون</span>
                </a>
                <a href="{{ route('admin.messages') }}" class="flex items-center gap-sm px-md py-sm rounded-lg {{ request()->routeIs('admin.messages') ? 'active-nav-bg text-secondary font-bold' : 'text-on-surface-variant font-medium' }} transition-all hover:bg-surface-container-high duration-200">
                    <span class="material-symbols-outlined">mail</span>
                    <span class="font-label-md text-label-md">الرسائل</span>
                </a>
                <a href="{{ route('admin.licenses') }}" class="flex items-center gap-sm px-md py-sm rounded-lg {{ request()->routeIs('admin.licenses') ? 'active-nav-bg text-secondary font-bold' : 'text-on-surface-variant font-medium' }} transition-all hover:bg-surface-container-high duration-200">
                    <span class="material-symbols-outlined">badge</span>
                    <span class="font-label-md text-label-md">رخص القيادة</span>
                </a>
                <a href="{{ route('admin.settings') }}" class="flex items-center gap-sm px-md py-sm rounded-lg {{ request()->routeIs('admin.settings') ? 'active-nav-bg text-secondary font-bold' : 'text-on-surface-variant font-medium' }} transition-all hover:bg-surface-container-high duration-200">
            ...
            <header class="h-16 flex items-center justify-between px-gutter bg-surface custom-shadow sticky top-0 z-40">
                ...
                <div class="flex items-center gap-md">
                    ...
                    <div class="flex items-center gap-xs">
                        <span class="font-label-sm text-on-surface-variant" id="header-time">{{ now()->format('h:i A') }}</span>
    ...
    <div id="mobileSidebar" class="fixed inset-0 z-50 hidden lg:hidden">
        ...
        <nav class="absolute right-0 top-0 h-full w-72 bg-surface-container-low border-l border-outline-variant p-lg overflow-y-auto">
            ...
            <div class="space-y-xs">
                <a href="{{ route('admin.dashboard') }}" class="flex items-center gap-sm px-md py-sm rounded-lg {{ request()->routeIs('admin.dashboard') ? 'active-nav-bg text-secondary font-bold' : 'text-on-surface-variant font-medium' }} transition-all hover:bg-surface-container-high"><span class="material-symbols-outlined">dashboard</span><span class="font-label-md">لوحة التحكم</span></a>
                <a href="{{ route('admin.cars') }}" class="flex items-center gap-sm px-md py-sm rounded-lg {{ request()->routeIs('admin.cars') ? 'active-nav-bg text-secondary font-bold' : 'text-on-surface-variant font-medium' }} transition-all hover:bg-surface-container-high"><span class="material-symbols-outlined">directions_car</span><span class="font-label-md">إدارة الأسطول</span></a>
                <a href="{{ route('admin.bookings') }}" class="flex items-center gap-sm px-md py-sm rounded-lg {{ request()->routeIs('admin.bookings') ? 'active-nav-bg text-secondary font-bold' : 'text-on-surface-variant font-medium' }} transition-all hover:bg-surface-container-high"><span class="material-symbols-outlined">calendar_month</span><span class="font-label-md">إدارة الحجوزات</span></a>
                <a href="{{ route('admin.control-center') }}" class="flex items-center gap-sm px-md py-sm rounded-lg {{ request()->routeIs('admin.control-center') ? 'active-nav-bg text-secondary font-bold' : 'text-on-surface-variant font-medium' }} transition-all hover:bg-surface-container-high"><span class="material-symbols-outlined">edit_note</span><span class="font-label-md">مركز التحكم</span></a>
                <a href="{{ route('admin.users') }}" class="flex items-center gap-sm px-md py-sm rounded-lg {{ request()->routeIs('admin.users') ? 'active-nav-bg text-secondary font-bold' : 'text-on-surface-variant font-medium' }} transition-all hover:bg-surface-container-high"><span class="material-symbols-outlined">group</span><span class="font-label-md">المستخدمون</span></a>
    ...
    <script>
        function updateTime() {
            ...
            document.getElementById('header-time').textContent = h12 + ':' + minutes + ' ' + ampm;
        }
        updateTime();
